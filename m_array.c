...
int main_multiplearray(void) {
	...
	*/
	// 프로젝트 10마리의 서로 다른 동물(각 카드 2장씩) 
	// 사용자로부터 2개의 입력값을 받음 -> 같은 동물 찾으면 카드 뒤집기
	// 모든 동물 쌍을 찾으면 게임 종료
	// 총 실패 횟수 알려주기
	srand(time(NULL));
...
int getEmptyPosition() {
	while (1) {
		int randPos = rand() % 20; // 0~19 사이의 숫자 반환, 19 -> {3,4} 좌표로 반환해야 함
...
int conv_pos_x(int x) {
	// 19 -> {3,4} 좌표로 반환해야 함, 5로 나누는 몫을 이용 
	return x / 5;
}
int conv_pos_y(int y) {
	// 19 -> {3,4} 좌표로 반환해야 함, 5로 나누고 나머지를 이용
	return y % 5;
}
void printAnimals() {
	printf("\n\n================이건 비밀인데.. 몰래 보여줍니다=================\n\n");
...
void printQuestion() {
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 5; j++) {
			// 카드를 뒤집어서 정답을 맞췄으면 "동물 이름" 표시
			if (checkAnimal[i][j] != 0) {
				printf("%8s", strAnimal[arrayAnimal[i][j]]);
			} // 틀렸으면 뒷면 -> 위치를 나타내는 숫자를 표시
			else {
				printf("%8d", seq);
			}
			seq ++;
